...
class JobEvaluation(BaseModel):    
    ...
    junior_score: float = Field(
        ge=0.0, le=100.0,
        description=(
            "Score from 0.0 to 100.0 based strictly on this formula: "
            "100.0 = Explicitly says 'Junior/No experience required' & 0 YOE. "
            "70.0-90.0 = 1-2 YOE required, standard tech stack. "
            "40.0-60.0 = 3 YOE or ambiguous mid-level requirements. "
            "1.0-3.0 = 4-5 YOE or has 'Senior' on title or description without lead duties. "
    ...
    language_llm: List[str] = Field(
       description=(
            "List of required languages WITH PROFICIENCY LEVELS extracted from the job description. "
            "MUST NOT be ISO country codes (do NOT use 'en', 'de', 'fr'). "
            "EXACT FORMAT REQUIRED: '[Language Name] [Level/Requirement]' "
            "(e.g., ['English B2', 'German Native/C1 Required', 'French Optional'])."
        )
    )
    ...
    foreign_friendly_score: float = Field(
        ...
        description=(
            ...
            "- Add 20 (cap 10) if 'Visa Sponsorship Available' or 'International applicants welcome' is explicitly stated. "
            "- If fully Remote (Global), score is 10 unless restricted by timezone/legal residency."
        )
    )
    ...
    llm_summary: str = Field(
        description=(
            ...
            "Don't use the CV or candidate profile in this summary. Focus solely on the job description content."
        )
    )

    # 1. Rationale FIRST
    cv_match_reasons: str = Field(
        description="Detailed plain text explanation of candidate fit vs missing requirements."
    )

    # 2. Strict Enum SECOND
    cv_match_rank: Literal["Fit", "Unfit", "Borderline", "Neutral", "Custom"] = Field(
        description="Select 'Unfit' if candidate lacks core domain requirements. Select 'Fit' if stack matches."
    )
